...
-- ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS trust_score NUMERIC(3,2) NOT NULL DEFAULT 0.00;
-- ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS fraud_score NUMERIC(3,2) NOT NULL DEFAULT 0.00;
-- ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS seller_score NUMERIC(6,2) NOT NULL DEFAULT 0.00;
-- ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS available_now BOOLEAN NOT NULL DEFAULT false;
-- ALTER TABLE public.shops ADD COLUMN IF NOT EXISTS last_seen_at TIMESTAMPTZ;
-- ALTER TABLE public.categories ADD COLUMN IF NOT EXISTS parent_slug TEXT REFERENCES public.categories(slug);
-- ALTER TABLE public.products ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'active';
-- ALTER TABLE public.products ADD COLUMN IF NOT EXISTS listing_score INTEGER NOT NULL DEFAULT 0;
...
-- 9. Listing Images (separate table with sort order)
...
-- 10. Listing Events (activity tracking)
...
-- 11. Lead Events (serious lead/contact tracking)
...
-- 12. Boost Plans (paid visibility packages)
...
-- 13. Listing Boosts (boost purchases applied to listings)
...
-- 14. Seller Reviews (buyer feedback)
...
-- 15. Fraud Flags (suspicious activity records)
...
-- 16. Notifications (user-facing messages)
...
BEGIN
    IF p_field = 'buyer_unread' THEN
        UPDATE public.conversations SET buyer_unread = buyer_unread + 1 WHERE id = p_conversation_id RETURNING buyer_unread INTO v_new;
    ELSE
        UPDATE public.conversations SET seller_unread = seller_unread + 1 WHERE id = p_conversation_id RETURNING seller_unread INTO v_new;
    END IF;
